...
public class GetWord {
    public static void main(String[] args) {
        ...
        try {
            InputStream is = new FileInputStream(new File("/Users/sephiroth/IdeaProjects/project/document_compare/doc/test1.doc")); //需要将文件路更改为word文档所在路径。
            ...
            for (int i = 0; i < range.numParagraphs() - 1; i++) {
                ...
                if (paratext1.length() > 0 && paratext2.length() > 0) {
                    ...
                    if (run1.getFontSize() > run2.getFontSize()
                            && i + 2  < range.numParagraphs()
                            && run2.getFontSize() > range.getParagraph(i + 2).getCharacterRun(0).getFontSize()) {
                        continue;
                    }
                    //连续两段字体格式不同
                    if (run1.getFontSize() > run2.getFontSize()) {
