package leetcode;

import java.util.*;

/**
 * 给你一个整数数组 nums ，判断是否存在三元组 [nums[i], nums[j], nums[k]] 满足 i != j、i != k 且 j != k ，同时还满足 nums[i] + nums[j] + nums[k] == 0 。请
 * 你返回所有和为 0 且不重复的三元组。
 * 注意：答案中不可以包含重复的三元组。
 *
 * 提示：
 * 3 <= nums.length <= 3000
 * -105 <= nums[i] <= 105
 */
public class _13_ThreeSums {

    public static void main(String[] args) {
        int[] arr = {34,55,79,28,46,33,2,48,31,-3,84,71,52,-3,93,15,21,-43,57,-6,86,56,94,74,83,-14,28,-66,46,-49,62,-11,43,65,77,12,47,61,26,1,13,29,55,-82,76,26,15,-29,36,-29,10,-70,69,17,49};
        System.out.println(threeSum(arr));
    }

    public static List<List<Integer>> threeSum(int[] nums) {
        Arrays.sort(nums); // 先排序

        List<List<Integer>> res = new ArrayList<>();

        // 因为是三数之和，只需要遍历到倒数第三个数字即可
        for (int i = 0; i < nums.length - 2; i++) {
            // 因为已经排过序，跳过相同的数字
            if(i > 0 && nums[i] == nums[i - 1]){
                continue;
            }
            int left = i + 1;
            int right = nums.length - 1;
            // 因为是三数之和，所以遍历第一个数时，后面两个数之和肯定等于第一个数的反值
            // a + b + c = 0 -> b + c = -a
            int target = -nums[i];
            // 使用双指针优化遍历
            while (left < right){
                if(nums[left] + nums[right] == target){
                    res.add(Arrays.asList(nums[i], nums[left], nums[right]));
                    left++;
                    right--;
                    // 第二层循环也需要跳过重复值
                    while (left < right && nums[left] == nums[left - 1])
                        left++; //继续左边过滤重复
                    while (left < right && nums[right] == nums[right + 1])
                        right--; //继续右边过滤重复
                } else if(nums[left] + nums[right] < target){
                    // 如果两数之和小于目标值, 则需要将左索引右移，因为排过序，需要找更大的值
                    left++;
                } else {
                    // 同上
                    right--;
                }
            }
        }
        return res;
    }
}

// [[-82, -11, 93], [-82, 13, 69], [-82, 17, 65], [-82, 21, 61], [-82, 26, 56], [-82, 33, 49], [-82, 34, 48], [-82, 36, 46], [-70, -14, 84], [-70, -6, 76], [-70, 1, 69], [-70, 13, 57], [-70, 15, 55], [-70, 21, 49], [-70, 34, 36], [-66, -11, 77], [-66, -3, 69], [-66, 1, 65], [-66, 10, 56], [-66, 17, 49], [-49, -6, 55], [-49, -3, 52], [-49, 1, 48], [-49, 2, 47], [-49, 13, 36], [-49, 15, 34], [-49, 21, 28], [-43, -14, 57], [-43, -6, 49], [-43, -3, 46], [-43, 10, 33], [-43, 12, 31], [-43, 15, 28], [-43, 17, 26], [-29, -14, 43], [-29, 1, 28], [-29, 12, 17], [-14, -3, 17], [-14, 1, 13], [-14, 2, 12], [-11, -6, 17], [-11, 1, 10], [-3, 1, 2]]
// [[-82, -11, 93], [-70, -14, 84], [-70, -6, 76], [-66, -11, 77], [-66, -3, 69], [-66, 1, 65], [-49, -6, 55], [-49, -3, 52], [-43, -14, 57], [-43, -6, 49], [-43, -3, 46], [-29, -14, 43], [-14, -3, 17], [-11, -6, 17], [-3, 1, 2]]
