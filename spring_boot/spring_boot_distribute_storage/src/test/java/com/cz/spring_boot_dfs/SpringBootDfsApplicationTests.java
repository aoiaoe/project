...
import com.cz.spring_boot_dfs.fastdfs.FastDFSService;
...
public class SpringBootDfsApplicationTests {
	...
	public void uploadFile(){
		...
		try(FileInputStream fileInputStream = new FileInputStream(new File(fileName))) {
			...
			System.out.println("上传后路径： " + fastDFSService.upload(bytes, fileName));
		}catch (Exception e){
			e.printStackTrace();
		}
	}
	...
	public void testDownload(){
		...
		byte[] download = this.fastDFSService.download(uploadedPath);
